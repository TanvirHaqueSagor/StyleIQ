...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _save(List<siq.Notification> list) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  Future<void> _markAllRead() async {
    final updated = _notifications.map((n) => n.markAsRead()).toList();
    await _save(updated);
    if (mounted) setState(() => _notifications = updated);
  }

  Future<void> _delete(siq.Notification n) async {
    final updated = _notifications.where((x) => x.id != n.id).toList();
    await _save(updated);
    if (mounted) setState(() => _notifications = updated);
  }
  ...
  Widget _buildTile(siq.Notification n, int index) {
    ...
    return Dismissible(
      ...
      child: GestureDetector(
        onTap: () => isUnread ? _markRead(n) : null,
        child: Container(
